...
export const EndpointRow = forwardRef<EndpointRowRef, Props>(
  ({ endpoint, isFastest, onMedianUpdate }, ref) => {
    const {
      median,
      start,
      stop,
      ...
      status,
      hasNetworkError,
    } = usePing(endpoint.url, endpoint.group || "GCP");
    ...
    return (
      <tr
        className={`
            border-b border-gray-100 last:border-0 transition-colors duration-200 cursor-pointer
            ${
              hasNetworkError
      ...
      >
        ...
        <td className="py-4 px-6">
          <a href={endpoint.url} target="_blank" rel="">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span
                  className={`text-sm font-medium ${isFastest ? "text-emerald-900" : "text-slate-700"}`}
                >
                  {endpoint.label}
                </span>
          ...
          <div className="flex flex-col items-end gap-1">
            {hasNetworkError ? (
              ...
              <div className="flex items-baseline gap-1">
                <span
                  className={`text-lg font-bold tabular-nums tracking-tight ${isFastest ? "text-emerald-600" : "text-slate-900"}`}
                >
                  {Math.round(median)}
                </span>
                <span
                  className={`text-xs font-medium ${isFastest ? "text-emerald-500" : "text-slate-400"}`}
                >
                  ms
                </span>
              </div>
